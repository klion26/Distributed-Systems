...
func Test1(t *testing.T) {
...

	{
		ck1.Ping(2)
		vx, _ := ck2.Ping(2)
		fmt.Println("Now[[[[[", vx.Viewnum, "]]]]] p[", vx.Primary, "] b[", vx.Backup, "]")
		for i := 0; i < DeadPings*2; i++ {
...

	{
		...
		// but do not ack
		vx, _ := ck1.Get()
		fmt.Println("iiiii", vx.Primary, vx.Backup, vx.Viewnum)
		for i := 0; i < DeadPings*3; i++ {
			ck1.Ping(0)
			ck3.Ping(vx.Viewnum)
			v, _ := ck1.Get()
		...
		vy, _ := ck1.Get()
		fmt.Println("JJJJ[", vy.Viewnum, vy.Primary)

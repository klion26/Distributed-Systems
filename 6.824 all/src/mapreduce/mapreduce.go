...
func InitMapReduce(nmap int, nreduce int,
	...
	/**
	  弄一个足够大缓冲区的 channel 保存可用的 worker
	 */
	mr.idleWorkerChannel = make(chan string, mr.nMap)
	return mr
}
...
// partitions.
/**
  增加了一个参数 mapDone ,为了并发,这个后期需要考虑其他的方式来实现
 */
...
// key
/**
   增加了一个参数 reduceDone, 为了并发,需要考虑其他的方式来写
 */
...
func (mr *MapReduce) Run() {
	...
	/**
	 * 这里才往 mr.DoneChannel 发送信号,表示所有任务都完成了
	 * 程序会在入口监控这个信号, 只有这个信号有值才退出
	 */
	mr.DoneChannel <- true
}
